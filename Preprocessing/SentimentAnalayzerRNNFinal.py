...
glove_model_name = ""
fasttext_model_name = "../../../../NoiseRemoval/trained_fastext_100/fasttext_100"
word2vec_model_name = "../../../../NoiseRemoval/trained_word2vec_100/word2vec_100.w2v"
...
def convert_to_vectors():
    comments = pd.read_csv("../../../corpus/analyzed/comments_no_punctuations_test.csv", ";")
...
def comments_to_vectors(data):
    ...
    #model = load_glove_model(glove_model_name)
    #model = load_fasttext_model(fasttext_model_name)
...
def get_sentence_vector_glove(model, sentence):
    ...
    for word in str(sentence).split(" "):
        if word in model.dictionary:
            print(word)
            sentence_vector[counter] = model.word_vectors[model.dictionary[word]]
            counter += 1
            if (counter == max_sentence_length):
                break
        else:
...
def get_sentence_vector_fasttext(model, sentence):
    sentence_vector = np.zeros([max_sentence_length, num_features])
    counter = 0
    for word in str(sentence).split():
        if word in model.wv.vocab:
            print(word)
            sentence_vector[counter] = np.copy(model.wv[word])
            counter += 1
            if (counter == max_sentence_length):
                break
        else:
...
def load_glove_model(gloveFile):
    model = Glove.load(gloveFile)
    return model

def load_fasttext_model(fasttextFile):
    model = fasttext.FastText.load(fasttextFile)
    return model
